...
const Singup = () => {
    const { githubSingIn, googleSingIn, singUpWithEmail, error, getEmail, getPassword } = useAuth();
    return (
        <div>
            ...
            <Form onSubmit={singUpWithEmail} className="w-25 mx-auto my-6">
                ...
                <button className="bg-green-400 py-2 px-5 mb-3 rounded-md text-white text-2xl">Regester</button><br />
                <Link to="/login">Already have a account?Log in</Link>
            </Form>

            <div className="mx-auto w-25">
                <div>----------You can Regester With:)-----------</div>

                <span onClick={googleSingIn} className="text-3xl bg-gray-400 inline-block p-2 rounded-lg mt-3 ms-5 cursor-pointer">
                    <FcGoogle />
                </span>
                <span onClick={githubSingIn} className="text-3xl bg-gray-400 inline-block p-2 rounded-lg mt-3 ms-5 cursor-pointer">
                    <FaGithub />
                </span>
            </div>
        </div>
